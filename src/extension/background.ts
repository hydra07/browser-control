...
function computeRedundantTextAxIds(
    byAxId: Map<string, AXNode>,
    meaningfulAxIds: Set<string>,
): Set<string> {
...
function buildSnapshotNodes(nodes: AXNode[]): SnapshotEntry[] {
    ...
    const redundantTextAxIds = computeRedundantTextAxIds(
        byAxId,
        new Set(kept.map((n) => n.nodeId)),
    );
    ...
    return kept
        .filter(
            (node) =>
                ...
                !redundantTextAxIds.has(node.nodeId),
        )
        .map(toCompactEntry);
}
...
function buildRegionTree(nodes: AXNode[]): {
    tree: SnapshotEntry[];
    truncated: boolean;
} {
    ...
    const byAxId = new Map<string, AXNode>(nodes.map((n) => [n.nodeId, n]));
    const meaningfulAxIds = new Set(
        nodes.filter(isMeaningfulAxNode).map((n) => n.nodeId),
    );
    const redundantTextAxIds = computeRedundantTextAxIds(
        byAxId,
        meaningfulAxIds,
    ...
    const root =
        nodes.find((n) => !referencedAsChild.has(n.nodeId)) ?? nodes[0];
...
function quadToBox(quad: Protocol.DOM.Quad): {
    x: number;
    y: number;
    w: number;
    h: number;
...
function showClickRipple(
    ...
    if (!document.getElementById("__bc_ripple_style__")) {
        ...
        style.textContent =
            "@keyframes __bc_ring__ { 0% { transform: scale(0.35); opacity: .85; } 100% { transform: scale(1); opacity: 0; } }";
        document.documentElement.appendChild(style);
    }
    const [a, b] =
...
async function getAxInfoForNode(
    target: chrome.debugger.Debuggee,
...
async function ensureOffscreenDocument(): Promise<void> {
    ...
    await chrome.offscreen.createDocument({
        ...
        justification:
            "Holds a persistent WebSocket connection to the local BrowserControl daemon. chrome.debugger is unavailable in offscreen documents, so this is transport only — the service worker still performs all CDP commands.",
    });
}
...
async function performPressKey(
    ...
    if (!def)
        return {
            ...
            hint: `Supported keys: ${Object.keys(KEY_DEFS).join(", ")}.`,
        };
...
async function takeFlowSnapshot(
    target: chrome.debugger.Debuggee,
...
async function dispatchCommand(
    ...
    if (cmd === "switch_tab") {
        try {
            ...
            return {
                error: `No tab with id ${data.tabId}`,
        ...
        return {
            success: true,
            message: `Switched to tab ${data.tabId}`,
            url: tab?.url,
            title: tab?.title,
        };
    }
    ...
    if (cmd === "query_region") {
        ...
        if (!queryResult?.nodeId) {
            return {
                error: `No element matched selector "${data.selector}"`,
        ...
        const axResult = await sendCommand(
            target,
    ...
    if (cmd === "visual_snapshot") {
        ...
        const toAnnotate = filteredNodes
            .filter((n) => n.i != null)
            .slice(0, MAX_ANNOTATED);
        ...
        await evalOnPage(
            target,
            `(${drawAnnotationOverlay.toString()})(${JSON.stringify(boxes)})`,
        );
    ...
    if (cmd === "inspect_element") {
        ...
        let wave1;
        try {
            wave1 = await Promise.all([
                sendCommand(target, "DOM.describeNode", { backendNodeId }),
                sendCommand(target, "DOM.getOuterHTML", { backendNodeId }),
                sendCommand(target, "DOM.pushNodesByBackendIdsToFrontend", {
                    backendNodeIds: [backendNodeId],
                }),
                sendCommand(target, "DOM.resolveNode", { backendNodeId }),
            ]);
        } catch {
            return {
        ...
        const [describeResult, outerHTMLResult, pushResult, resolveResult] =
            wave1;
        ...
        if (
            ...
            outerHTML.length > MAX_HTML_CHARS
        ) {
        ...
        const [matchedResult, computedResult, listenersResult] =
            await Promise.all([
                nodeId != null
                    ? sendCommand(target, "CSS.getMatchedStylesForNode", {
                          nodeId,
                      })
                    : Promise.resolve(null),
                nodeId != null
                    ? sendCommand(target, "CSS.getComputedStyleForNode", {
                          nodeId,
                      })
                    : Promise.resolve(null),
                objectId
                    ? sendCommand(target, "DOMDebugger.getEventListeners", {
                          objectId,
                      })
                    : Promise.resolve(null),
            ]);
        ...
        if (objectId)
            void sendCommand(target, "Runtime.releaseObject", {
                objectId,
            }).catch(() => {});
...
async function handleNavigate(url: string): Promise<Record<string, unknown>> {
    ...
    if (activeTabId) {
        try {
            ...
            console.log(
                `Stale activeTabId ${activeTabId} (tab no longer exists) — creating a new tab.`,
            );
            activeTabId = null;
            isDebuggerAttached = false;
        }
    ...
    if (!existingTabIsValid) {
        ...
        const updatedTab = await chrome.tabs.update(activeTabId!, {
            url: url,
            active: true,
        });
