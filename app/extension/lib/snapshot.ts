// Builds the compact {i,r,n,v} node list browser_snapshot/browser_query_region/
// browser_visual_snapshot return, from a raw CDP accessibility tree.
...
// Same nodes as browser_snapshot, plus a screenshot with a numbered box over
// every interactive element so an id can be grounded to a screen position
// before clicking, instead of guessing from text alone.
export async function handleVisualSnapshotCommand(
    target: chrome.debugger.Debuggee,
): Promise<Record<string, unknown>> {
    const axTreeResult = await sendCommand(
        target,
