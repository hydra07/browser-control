...
// evalOnPage/sendCommand or anything else in this module. Draws a numbered
// box over every interactive element passed in, so a node id can be grounded
// to a position on screen before clicking, instead of guessing from text
// alone. Same brand palette (indigo/violet) as the click/type visual
// feedback in background.ts, for a consistent "an AI is doing this" identity.
...
// Self-contained: injected via .toString(), see drawAnnotationOverlay above.
export function removeAnnotationOverlay() {
    document.getElementById("__bc_annotate_overlay__")?.remove();
}

// Plain viewport/full-page screenshot (the `screenshot` command). fullPage
// switches Page.captureScreenshot's clip to the full document content size
...
// Draws the numbered annotation overlay, captures a screenshot of it, then
// removes the overlay again — used by the `visual_snapshot` command. Caller
// is responsible for computing `boxes` (via DOM.getBoxModel) before calling.
