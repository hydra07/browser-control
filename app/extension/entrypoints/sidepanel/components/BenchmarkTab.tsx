...
import { type BenchmarkMetrics, getMetrics } from "../lib/api";
import {
  ChartBarIcon,
  CheckIcon,
  ChevronDownIcon,
...
const DEFAULT_SUMMARY: BenchmarkMetrics["summary"] = {
  sessionId: "—",
...
const DEFAULT_SAVINGS: BenchmarkMetrics["tokenSavings"] = {
...
export function BenchmarkTab({ onRefresh }: BenchmarkTabProps) {
  ...
  const tokenSavings = metrics?.tokenSavings ?? DEFAULT_SAVINGS;
    ...
    <div className="flex flex-1 flex-col overflow-y-auto bg-[#0d0e12] p-3 text-zinc-200 font-sans select-text">
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="rounded-lg border border-zinc-800/90 bg-[#121318] p-2.5 shadow-sm">
          ...
          <div className="mt-1 flex items-baseline gap-1.5">
            ...
            <span className="text-[10px] font-mono text-zinc-500">(~{summary.avgTokensPerCall}/call)</span>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-zinc-800/90 bg-[#121318] p-2.5 shadow-sm">
          ...
          <div className="mt-1 flex items-baseline gap-1.5">
            <span className="font-mono text-base font-bold text-zinc-100">{summary.totalCalls}</span>
            {summary.flowStepTotal > 0 && (
              <span className="text-[10px] font-mono text-emerald-400">({summary.flowStepTotal} steps)</span>
            )}
          </div>
        </div>
        ...
        <div className="rounded-lg border border-zinc-800/90 bg-[#121318] p-2.5 shadow-sm">
          ...
          <div className="mt-1 flex items-baseline gap-1.5">
            <span className="font-mono text-base font-bold text-zinc-100">{summary.avgDurationMs}</span>
          ...
          <div className="grid grid-cols-3 gap-1.5 pt-1 text-[10px] font-mono">
            <div className="rounded bg-zinc-800/50 p-1 text-center">
              ...
              <div className="text-emerald-400 font-bold">
                ~{formatTokens(tokenSavings.savingsBreakdown.fromFlowBatching)}
              </div>
            </div>
            <div className="rounded bg-zinc-800/50 p-1 text-center">
              <div className="text-zinc-500">Compact Snap</div>
              <div className="text-emerald-400 font-bold">
                ~{formatTokens(tokenSavings.savingsBreakdown.fromCompactSnapshots)}
              </div>
            </div>
            <div className="rounded bg-zinc-800/50 p-1 text-center">
              <div className="text-zinc-500">Docs Blocks</div>
              <div className="text-emerald-400 font-bold">
                ~{formatTokens(tokenSavings.savingsBreakdown.fromDocsBlocks)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mb-3">
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <span className="font-mono text-[10px] text-zinc-500">{byCommand.length} actions</span>
        </div>
      ...
      <div className="mb-2">
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <span className="font-mono text-[10px] text-zinc-500">Last {recentCalls.length} calls</span>
        </div>
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-1.5 font-mono min-w-0">
                      ...
                      {call.argsSummary && (
                        <span className="text-[10px] text-zinc-500 truncate max-w-[100px]">{call.argsSummary}</span>
                      )}
                    </div>
                  ...
                  {isExpanded && (
                    <div className="border-t border-zinc-800/80 bg-[#0a0b0e] p-2 text-[10px] font-mono">
                      <div className="flex justify-between text-zinc-500 mb-1">
                        ...
                        <span>
                          Total: ~{call.approxTokens} tok ({call.inTokens} in / {call.outTokens} out)
                        </span>
