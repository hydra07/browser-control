/**
 * Mathematical geometry and trajectory calculation engine for drag actions.
 * Supports lines, circles, arcs, ellipses, Bezier curves, spirals, and waves.
 */
...
export function generateWaypointsPath(cfg: TrajectoryConfig, fallbackCenter?: Point): Point[] {
    ...
    if (!cfg.smoothing) {
        ...
        for (let i = 0; i < pts.length - 1; i++) {
            const seg = generateStraightPath(
                pts[i],
                pts[i + 1],
                Math.max(3, Math.round((cfg.steps ?? 30) / (pts.length - 1))),
            );
...
export function generatePolarPath(cfg: TrajectoryConfig, fallbackCenter?: Point): Point[] {
    ...
    const endAng = normalizeAngle(
        cfg.endAngle ?? cfg.tMax,
        clockwise ? startAng + Math.PI * 2 : startAng - Math.PI * 2,
    );
