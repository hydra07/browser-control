    ...
    const matches = await Promise.all(
        nodeIds.map(async (nodeId) => {
            ...
            const [axInfo, resolveResult] = await Promise.all([
                getAxInfoForNode(target, backendNodeId).catch((): AxInfo => ({})),
...
/** Self-contained element to markdown serializer for injected page evaluation. */
...
export async function handleSelectContentCommand(
    ...
    if (opts.nodeId != null) {
        ...
        const value = res?.result?.value as { count: number; blocks: string[] } | undefined;
