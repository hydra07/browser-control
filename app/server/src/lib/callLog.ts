...
import { recordToolCallDetail } from "./dataStore.js";
...
function writeCallLog(logFile: string, entry: CallLogEntry): void {
  ...
  console.error(`[tool:${entry.source}] ${entry.cmd} ${entry.durationMs}ms ~${entry.inTokens}in/${entry.outTokens}out tok${entry.hasImage ? ' [image]' : ''}${entry.isError ? ' ERROR' : ''}`);
...
export function logToolCall(logFile: string, name: string, args: ToolArgs, response: ToolCallResponse, durationMs: number): void {
  ...
  for (const item of response?.content ?? []) {
    if (item.type === "text") { outChars += item.text?.length ?? 0; text += item.text ?? ""; }
    if (item.type === "image") { outChars += item.data?.length ?? 0; hasImage = true; }
  }
