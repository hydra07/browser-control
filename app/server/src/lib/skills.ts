 ...
 */
export function saveSkill(args: { name?: unknown; domains?: unknown; description?: unknown; content?: unknown }): Record<string, unknown> {
  ...
  if ((!domains || domains.length === 0) || description === undefined) {
    ...
    if (!domains || domains.length === 0) {
      if (existing) {
        ...
        return { error: "Missing domains", hint: "New skills need at least one domain (e.g. [\"github.com\"]) so browser_session's navigate action can find them. Omit only when updating a skill that already has domains set." };
      }
    }
