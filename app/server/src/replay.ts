...
/**
 * Moved under data/ (used to be a top-level logs/ dir) — daemon.ts migrates
 * any legacy logs/*.jsonl into the new location on startup, but replay can
 * run without the daemon ever having been started against this checkout, so
 * fall back to the legacy path if the new one doesn't exist.
 */
...
/** The compact shape browser_inspect's snapshot/visual_snapshot return per element (see background.ts's toCompactEntry): i=nodeId, r=role, n=name. */
...
function listSessions(): void {
  ...
  try {
    ...
    console.log("No logs/ directory yet — run a session through the MCP tools first.");
    return;
  }
  if (files.length === 0) {
  ...
  for (const f of files) {
    ...
    console.log(`${f}  (${lineCount} calls, ${(statSync(full).size / 1024).toFixed(1)} KB)`);
  }
}
 ...
 * navigate reassigns them all, so replaying an old click's raw id is a coin
 * flip at best (see the header comment). role+name survive a reload because
 * they reflect the page's actual content, so when a logged entry has them
 * (background.ts's getAxInfoForNode attaches these to every click/type
...
async function replay(logPath: string, opts: { continueOnError: boolean; delayMs: number }): Promise<void> {
  const resolvedPath =
    logPath.startsWith("logs/") || logPath.startsWith("data/logs/") || logPath.includes(":") || logPath.startsWith("/")
      ? logPath
  ...
  for (let i = 0; i < lines.length; i++) {
     ...
     * navigate, ...) the daemon's HTTP relay expects, not the gateway tool
     * name that carried it. The "browser_" strip below is now a no-op kept
     * only for log files recorded before the gateway-tool refactor, whose
     * cmd field still has the pre-refactor per-action MCP tool name.
     */
    ...
    try {
      ...
      console.log("Is the daemon running with the extension connected? (bun run src/server/daemon.ts)");
...
function parseDelay(argv: string[]): number {
  ...
  const n = nextArg ? parseInt(nextArg, 10) : NaN;
  return Number.isFinite(n) ? n : DEFAULT_DELAY_MS;
}

const firstArg = argv[0];
if (argv.includes("--list") || !firstArg) {
  listSessions();
} else {
  const logPath = firstArg;
  const continueOnError = argv.includes("--continue");
  const delayMs = parseDelay(argv);
  console.log(`(delay between steps: ${delayMs}ms — pass --delay 0 for no pause, or --delay=N)`);
