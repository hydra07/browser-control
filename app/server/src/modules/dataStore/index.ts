...
try {
  db.run(`ALTER TABLE tool_calls ADD COLUMN bun_rss_mb REAL DEFAULT 0`);
} catch {}
try {
  db.run(`ALTER TABLE tool_calls ADD COLUMN bun_heap_used_mb REAL DEFAULT 0`);
} catch {}
try {
  db.run(`ALTER TABLE tool_calls ADD COLUMN bun_heap_total_mb REAL DEFAULT 0`);
} catch {}
try {
  db.run(`ALTER TABLE tool_calls ADD COLUMN ext_heap_used_mb REAL DEFAULT 0`);
} catch {}
try {
  db.run(`ALTER TABLE tool_calls ADD COLUMN ext_heap_total_mb REAL DEFAULT 0`);
} catch {}
try {
  db.run(`ALTER TABLE tool_calls ADD COLUMN ext_listeners INTEGER DEFAULT 0`);
} catch {}
try {
  db.run(`ALTER TABLE tool_calls ADD COLUMN ext_cache_entries INTEGER DEFAULT 0`);
} catch {}
try {
  db.run(`ALTER TABLE tool_calls ADD COLUMN ext_duration_ms INTEGER DEFAULT 0`);
} catch {}
  ...
  try {
    ...
    db.query(
      `INSERT INTO tool_calls (
        ...
        step_count, created_at, bun_rss_mb, bun_heap_used_mb, bun_heap_total_mb,
        ext_heap_used_mb, ext_heap_total_mb, ext_listeners, ext_cache_entries, ext_duration_ms
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      input.sessionId,
      input.cmd,
      ...
      input.createdAt ?? Date.now(),
      input.bunRssMb ?? 0,
      input.bunHeapUsedMb ?? 0,
      input.bunHeapTotalMb ?? 0,
      input.extHeapUsedMb ?? 0,
      input.extHeapTotalMb ?? 0,
      input.extListeners ?? 0,
      input.extCacheEntries ?? 0,
      input.extDurationMs ?? 0,
    );
  } catch (e) {
...
export function getBenchmarkMetrics(sessionId?: string): BenchmarkMetrics {
  ...
  const rows = db
    ...
    .all(activeSessionId) as RawCallRecord[];

  const docsRow = db.query(`SELECT COUNT(*) as count FROM docs_blocks WHERE session_id = ?`).get(activeSessionId) as
    | { count: number }
    | undefined;

  return BenchmarkEngine.computeMetrics({
    rows,
    sessionId: activeSessionId,
    sessionName,
    startedAt,
    docsBlocksCount: docsRow?.count ?? 0,
  });
}
